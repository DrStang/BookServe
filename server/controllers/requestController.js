...
async function getOcean(title, author) {
...
async function processBookRequest(requestId) {
    try {
        ...
        const ocean = await getOcean(request.title, request.author);
        ...
        if (anna) {
            ...
            console.log(`[Process] AA failed, Ocean updating status to completed for request ${requestId}`);
            await BookRequest.updateStatus(requestId, 'completed');
            await folderScanService.triggerScan();
            console.log(`[Process] Status updated successfully`);
        } else {
            console.log('[Process] Ocean failed, trying NZB');
