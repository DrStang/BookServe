...
class RetryService {
  ...
  async retryBookSearch(request) {
    try {
      ...
      // Use the improved search function from requestController
      const searchResults = await searchNZBHydra(
        request.title,
        request.author,
        request.isbn,
        request.id
      ...
      console.log(`[Retry] Found ${searchResults.length} result(s) for: "${request.title}"`);
...
Match Score: ${searchResult.relevanceScore}
