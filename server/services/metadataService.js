...
const goodreadsMetadata = require('./goodreadsMetadata');
...
class MetadataService {
    ...
    async fetchMetadata(bookInfo) {
        ...
        try {
            ...
            const [googleData, openLibraryData] = await Promise.all([
                ...
                const goodreadsMeta = await goodreadsMetadata.getMetadata(bookInfo).catch(err => {
     ...
     */
    mergeMetadata(googleData, openLibraryData, goodreadsData, goodreadsMeta, originalData) {
        ...
        setValue('title', goodreadsMeta, googleData, openLibraryData);
        setValue('subtitle', googleData, openLibraryData);
        setValue('author', goodreadsMeta, googleData, openLibraryData);
        setValue('description', goodreadsMeta, googleData, openLibraryData);
        setValue('isbn', googleData, openLibraryData);
        setValue('isbn_13', googleData, openLibraryData);
        setValue('publisher', goodreadsMeta, googleData, openLibraryData);
        setValue('published_date', goodreadsMeta, googleData, openLibraryData);
        setValue('page_count', goodreadsMeta, googleData, openLibraryData);
        setValue('language', goodreadsMeta, googleData, openLibraryData);
        setValue('categories', goodreadsMeta, googleData, openLibraryData);
        setValue('preview_link', goodreadsMeta, googleData);
        ...
        if (Array.isArray(merged.categories)) {
            merged.categories = merged.categories.join(' , ');
        }    
        ...
        }   else if (goodreadsMeta?.average_rating && goodreadsMeta?.ratings_count) {
            merged.average_rating = goodreadsMeta.average_rating;
            merged.ratings_count = goodreadsMeta.ratings_count;
